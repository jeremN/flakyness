...
- **TypeScript is split across the workspace**: `apps/api` is on **TS 7**; `apps/dashboard` stays on **TS 6** because `svelte-check` 4.x crashes under TS 7 — it reads `ts.default.sys.useCaseSensitiveFileNames`, a CommonJS shape the native TS 7 rewrite removed. That crash is a symptom, not the root cause: verified 2026-07-15, TS 7.0 GA ships no stable *programmatic* API, and Svelte's template type-checking (what `svelte-check` does for code outside `.ts` files) needs that API to run at all against the Go (`tsgo`) compiler — so Svelte (and Vue/Astro/MDX) simply cannot type-check on TS 7.0, upstream bug, not fixable here. The programmatic API is expected in **TypeScript 7.1 (~Oct 2026, an upstream estimate, not a promise)**; official Svelte guidance is to stay on TS 6 until then. `.github/dependabot.yml` has a dashboard-only entry that ignores TypeScript majors; only lift it once BOTH TS 7.1 has shipped AND a `svelte-check` release supports it (latest is still 4.7.2 as of this writing) — track `sveltejs/language-tools#2733`.
- **`pnpm --filter dashboard check` reports 3 phantom errors in a shell where you exported `.env`.** `svelte-kit sync` generates the `$env/dynamic/private` `env` type from the *ambient environment at sync time*, declaring every variable it finds as a **required** `string` — the generated `.svelte-kit/ambient.d.ts` will even contain your unrelated shell vars. With `ADMIN_TOKEN` exported the property stops being optional, so `src/lib/server/adminApi.test.ts`'s three `delete privateEnv.ADMIN_TOKEN` lines each raise TS2790 *"The operand of a 'delete' operator must be optional"*. In a clean shell the same 479 files report **0 errors**. CI is unaffected — its Type Check job doesn't export `.env` (only the Tests job does), so PRs are green and correctly so. The trap: this guide tells you to export `.env` for the API suites, and then the documented typecheck command fails for reasons unrelated to your change. Re-run as `env -u ADMIN_TOKEN pnpm --filter dashboard check` before believing it. Verified 2026-08-14 by toggling only the export, nothing else.
