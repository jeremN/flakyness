...
- **New projects default to unassigned, and that default predates this plan.** `teamId` is optional on `POST /api/v1/admin/projects` (plan 057); unlike migration 0012's one-time backfill of pre-existing rows, nothing assigns a team to a project created afterward with `teamId` omitted. On the live dev DB, 1529 of 2952 projects have `team_id IS NULL`, virtually all created after the migration — this is not team-deletion orphaning, it is the create-time default. Combined with the constraint above, that means a project created without `teamId` becomes invisible, by default, to the very team that created it. This plan must decide that behavior deliberately rather than silently inherit it: options include rejecting a create with no `teamId`, defaulting to a fallback team, or keeping today's default but surfacing "unassigned" prominently in the dashboard (plan 059) so it stops being a silent trap.
- **The last-admin guard's TOCTOU must be closed in the SAME change that strips `ADMIN_TOKEN` of superuser status — not before, not after.** `admin-users.ts`'s delete and demote paths do a read-then-write global-admin count check with no lock; plan 057's review deferred it as Minor on the grounds that zero global admins is "unrecoverable." That reasoning was wrong for plan 057: today, `PATCH /admin/users/:id {"isGlobalAdmin": true}` is gated by `ADMIN_TOKEN` alone with no guard, so recovery from zero global admins is a single curl, not a database edit. It becomes genuinely unrecoverable only once this plan requires a global-admin *session* (rather than `ADMIN_TOKEN`) for admin routes — at that point a race that leaves zero global admins locks every operator out of the API layer with no break-glass path left. Land the transactional guard (`SELECT … FOR UPDATE`, or a serializable transaction around the count-then-write, in both the delete and demote paths) in the same change that removes `ADMIN_TOKEN`'s standing superuser status. Landing it earlier is inert (nothing depends on it yet); landing it later leaves a real window open.
- **`projects.team_id` has NO index** (noted in plan 057's Task 1+2 review; `team_members` indexes its equivalent FK, `projects` does not). Deliberate YAGNI: `projects` holds one row per tracked repository, so the sequential scan this plan's `WHERE projects.team_id = …` predicates trigger is cheap at the product's real shape. **If profiling here says otherwise, adding the index means relaxing the "no migration" constraint above** — decide that explicitly rather than discovering it in production.
